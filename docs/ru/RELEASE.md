...
Метаданные реестра и индексы установки могут обновляться в разное время. Каждая установка npm/Python повторяется после неудачной попытки с паузой 10 секунд и общим лимитом 120 секунд, включая выполнение команд. Pip не использует локальный кеш. Исчерпание лимита завершает job ошибкой; consumer-примеры запускаются один раз после установки и сразу завершают job ошибкой при нарушении проверок.
